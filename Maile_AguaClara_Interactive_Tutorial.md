...
My name is Maile and I am a junior at Cornell. I like Led Zeppelin and sustainable engineering.
...
Using a Relative File Path:
![Pupper](\Images\Maile\Pictures\dog.jpg)
...
<!--- Fill you answer here. --->
My major is Civil Engineering, which is the study of integrating designs into the natural environment, building roads, bridges, energy infrastructure, water systems and more. [http://www.cee.cornell.edu/]()
...
<!--- Fill you answer here. --->
| animal        | food           | book                           | place on campus   |
|:------------- |:-------------- | ------------------------------ | ----------------- |
| polar bear    | sweet potatoes | the picture of dorian grey     | top of libe slope |
| llama         | salmon         | fear and loathing in las vegas | psb atrium        |
| komodo dragon | ice cream      | Slouching Toward Bethlehem     | johnson museum    |
...
my2DArray = np.array([[1 ,2, 3, 4, 5], [4, 5, 6, 7, 8], [7, 8, 9, 10, 11]])
my2DArray[1,:]
my2DArray[:,1]
np.size(my2DArray)
m2DArrayUnits = my2DArray*u.L
```
...
def stokes(eta, T, r)
  ...
  new_T = T.to(u.kelvin)
  etaUnits = eta.to(u.kilogram/ (u.meter * u.second))
  new_r = r.to(u.meter)
  D = (kB*newT)/(6* np.pi * new_r *etaUnits)
  return D.to.base.units()
```
...
tempAr = (np.array(temp) + 273) *u.kelvin

viscosity = []
for i in range(len(tempArray)):
  calc_visc = pc.viscosity_kinematic(tempAr)
  viscosity.append(calc_visc)
viscosity

num = []
d = .2 * 2 * u.meter
flow = 2 * (u.meter ** 3 / u.s)

for j in range(len(viscosity)):
  re = pc.re_pipe(d, flow, viscosity[j])
  re_number.append(re)

plt.plot(tempAr, num, '.', label = Re Number)
plt.ylabel('Reynolds Number')
plt.xlabel('Temperature')
plt.title('Reynolds Number vs Temperature Change')
...
Ayo get it the grind doesnt stop -Steven Lopez
